...
class ExplainImportance:
    ...
    def get_importance(self):
        ...
        self.explainer = TreeShap(self.model,
                                  task = "regression",
                                  model_output = "raw")

        self.explainer.fit()

    def plot_importance(self, X, n_features, class_name, axis = 0, summary = "median", **kwargs):
        """
        ...
        feat_imp_sorted_index = feat_imp_pd.abs().sort_values(ascending = False).index.values[0:n_features]
        feat_imp_sorted_data = feat_imp_pd[feat_imp_sorted_index].values

        if X.ndim == 1:
            feat_values = pd.Series(data = X, index = self.feature_names)[feat_imp_sorted_index].values

            feat_imp_sorted_index = np.asarray(["%s (%.2f)" %(f_name, feat_values[i]) for i, f_name in enumerate(feat_imp_sorted_index)])

        # Get colors of bars
        bar_cols = ["Orange" if score < 0 else "Green" for score in feat_imp_sorted_data]
        ...
        if overview == "Global":
            plt.title("TreeOrdination %s Feature Importance Plot for Class %s" %(overview, str(class_name)))

        else:
            plt.title("TreeOrdination %s Feature Importance Plot\nPredicted Model Output for Sample = %.2f" %(overview, prediction))
